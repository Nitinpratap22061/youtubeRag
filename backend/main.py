...
# ========== Transcript Fetch (Corrected) ==========
def fetch_transcript_text(video_id: str) -> str:
    """Fetch transcript text for a given YouTube video ID with robust retry logic."""
    retries = 6
    ...
    for attempt in range(retries):
        try:
            ...
            if "Too Many Requests" in str(e) or "Login Required" in str(e):
                if attempt < retries - 1:
                    ...
                    raise HTTPException(status_code=500, detail=f"Transcript fetch failed after multiple retries due to rate limit: {e}")
            else:
